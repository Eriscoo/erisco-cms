...
import { getPublicPostsByTag, type Post } from '../../modules/posts/api'
...
function TagPosts({ navigate, tag }: Props) {
  const { t } = useLocale()
  ...
  const fetchPosts = useCallback(async (pageNum: number) => {
    try {
      setLoading(true)
      const data = await getPublicPostsByTag(tag, pageNum, limit)
      setPosts(data.posts)
      setTotal(data.total)
      setError(null)
      setNotFound(false)
      ...
      if (status === 404) {
        ...
        setError(err instanceof Error ? err.message : t.tagPosts.failed)
        setNotFound(false)
      }
    } finally {
      setLoading(false)
      ...
      <div className="flex-1 max-w-[1280px] w-full mx-auto px-4 md:px-8 py-8 md:py-12">
        ...
        {loading && posts.length === 0 ? (
          ...
          <div className="flex flex-col items-center gap-4 py-20">
            ...
            <Button variant="outline" size="sm" onClick={() => fetchPosts(1)}>{t.tagPosts.retry}</Button>
          </div>
        ) : posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 md:py-24">
            ...
            <div className="space-y-2 max-w-sm text-center">
              <h3 className="text-xl md:text-2xl font-bold text-white">{t.tagPosts.noPosts}</h3>
              <p className="text-zinc-400 text-sm leading-relaxed">
                {t.tagPosts.noPostsSub}
              </p>
            </div>
                          ...
                          {(() => {
                            ...
                            return tags.map((tg, i) => (
                              <span
                                ...
                                onClick={(e) => { e.stopPropagation(); navigate(`/posts/tags/${encodeURIComponent(tagSlugs[i] || tg)}`) }}
            ...
            <div className="mt-6">
              {error ? (
                <div className="flex flex-col items-center gap-3">
                  <p className="text-pink-400 text-sm">{error}</p>
                  <Button variant="outline" size="sm" onClick={() => fetchPosts(page)}>{t.tagPosts.retry}</Button>
                </div>
              ) : (
